...
__all__ = [
    "TrainValidImageDataset", "TrainValidImageFromVideoDataset", "TestImageDataset",
    "PrefetchGenerator", "PrefetchDataLoader", "CPUPrefetcher", "CUDAPrefetcher", "TestImageFromVideoDataset"
]

class TestImageFromVideoDataset(IterableDataset):
    ...
    def __init__(
        ...
        super(TestImageFromVideoDataset, self).__init__()
        self.video_file_names = [os.path.join(gt_video_dir, image_file_name) for image_file_name in
                                 os.listdir(gt_video_dir) if image_file_name.endswith(".webm")]
        self.gt_image_size = gt_image_size
        self.upscale_factor = upscale_factor
        self.mode = mode
        self.video_file_idx = 0
        # self.vidcap = cv2.VideoCapture(self.video_file_names[self.video_file_idx])

    def __iter__(self):

        for video_file in self.video_file_names:
            ...
            while success:
                ...
                # gt_crop_image = cv2.imread(image).astype(np.float32) / 255.
                gt_image = image / 255.0
                ...
                for i in range(9):
                    for j in range(16):
                        gt_crop_image = gt_image[80*i:80*(i+1), 80*j:80*(j+1)]
                        ...
                        yield {"gt": gt_crop_y_tensor, "lr": lr_crop_y_tensor}
            
            vidcap.release()

    def __len__(self) -> int:
        total_frames = 0
        for name in self.video_file_names:
            vidcap = cv2.VideoCapture(name)
            total_frames += int(vidcap.get(cv2.CAP_PROP_FRAME_COUNT))
            vidcap.release()
        return total_frames

class TrainValidImageFromVideoDataset(IterableDataset):
    """Define training/valid dataset loading methods.
    ...
    def __init__(
        ...
        super(TrainValidImageFromVideoDataset, self).__init__()
        self.video_file_names = [os.path.join(gt_video_dir, image_file_name) for image_file_name in
                                 os.listdir(gt_video_dir) if image_file_name.endswith(".webm")]
        self.gt_image_size = gt_image_size
        self.upscale_factor = upscale_factor
