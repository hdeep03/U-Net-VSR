...
def main() -> None:
    ...
    # g_model.state_dict(checkpoint["state_dict"])
    ...
    for index in range(total_files):
        ...
        # print(gt_y_tensor.shape, gt_y_tensor.max())
        # Only reconstruct the Y channel image data.
        # with torch.no_grad():
        #     sr_y_tensor = g_model(lr_y_tensor)
        # print(lr_y_tensor.shape)
        sr_y_tensor = torch.zeros((1, 1, 720, 1280)).cuda()
        for i in range(0, 180, 20):
            for j in range(0, 320, 20):
                with torch.no_grad():
                    sr_y_tensor[:,:,i*4:i*4+80,j*4:j*4+80] = g_model(lr_y_tensor[:,:,i:i+20,j:j+20])
        


        us_y_tensor = torch.nn.functional.interpolate(lr_y_tensor, scale_factor=4, mode='bicubic', align_corners=False)
        # Save image
        sr_y_image = imgproc.tensor_to_image(sr_y_tensor, range_norm=False, half=True)
        us_y_image = imgproc.tensor_to_image(us_y_tensor, range_norm=False, half=True)
        sr_y_image = sr_y_image.astype(np.float32) / 255.0
        us_y_image = us_y_image.astype(np.float32) / 255.0
        sr_ycbcr_image = cv2.merge([sr_y_image, gt_cb_image, gt_cr_image])
        us_ycbcr_image = cv2.merge([us_y_image, gt_cb_image, gt_cr_image])
        sr_image = imgproc.ycbcr_to_bgr(sr_ycbcr_image)
        us_image = imgproc.ycbcr_to_bgr(us_ycbcr_image)
        cv2.imwrite(sr_image_path, sr_image * 255.0)
        us_image_path = os.path.join("/home/alex/U-Net-VSR/data/raw/us_images", file_names[index])
        cv2.imwrite(us_image_path, us_image * 255.0)


        # Cal IQA metrics
        psnr_metrics += psnr(sr_y_tensor, gt_y_tensor).item() - psnr(us_y_tensor, gt_y_tensor).item()
        ssim_metrics += ssim(sr_y_tensor, gt_y_tensor).item() - ssim(us_y_tensor, gt_y_tensor).item()
